...
import AnimatedStickerKeyboard from "./AnimatedKeyBoard";
...
export default class AnimatedSticker {
  ...
  static setExtraCategory(category) {
    ...
    m === 0
      ? (extras.cat = category)
      : console.warn(
          `Can't use reserved id in setExtraCategory make an id start from 101 : ${JSON.stringify(
            data
          )} `
        );
  }
  ...
  static setExtraEmojis(items) {
    ...
    m === 0
      ? (extras.emoji = items)
      : console.warn(
          `Can't use reserved id in setExtraEmojis\nMake sure that id start from 3001 : ${JSON.stringify(
            data
          )}`
        );
  }
